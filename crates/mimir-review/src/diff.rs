...
pub fn parse_diff(diff_text: &str, base: &Utf8Path) -> Vec<FileDiff> {
    ...
    for line in diff_text.lines() {
        if let Some(caps) = file_header_re.captures(line) {
            ...
            let old_lines = caps.get(2).and_then(|m| m.as_str().parse::<u32>().ok()).unwrap_or(0);
            let new_start = caps[3].parse::<u32>().unwrap_or(0);
            let new_lines = caps.get(4).and_then(|m| m.as_str().parse::<u32>().ok()).unwrap_or(0);
...
fn is_generated_file(path: &str, base: &Utf8Path) -> bool {
    ...
    if let Ok(content) = std::fs::read_to_string(&full_path) {
        if content.contains("Generated by") || content.contains("DO NOT EDIT") || content.contains("Auto-generated") {
            return true;
        }
    }
...
pub fn detect_uninspected(
    diffs: &[FileDiff],
    inspected_paths: &HashSet<String>,
) -> Vec<Finding> {
